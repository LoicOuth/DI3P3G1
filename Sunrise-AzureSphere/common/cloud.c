...
static Cloud_TelemetryUploadEnabledChangedCallbackType
    chauffeEauEnabledChangedCallbackFunction =
...
Cloud_Result Cloud_SendChauffeEauEnabledChangedEvent(bool uploadEnabled,
                                                                     bool fromCloud)
{
    ...
    json_object_dotset_boolean(thermometerTelemetryUploadRoot,
                               "chauffeEauEnabled.value", uploadEnabled ? 1 : 0);
...
static void DeviceTwinCallbackHandler(const char *nullTerminatedJsonString)
{
    ...
    // If we have a desired property for the "chauffeEauEnabled" property, let's
    // process it.
    if (rootObject != NULL) {

        int chauffeEauEnabledValue =
            json_object_dotget_boolean(desiredProperties, "chauffeEauEnabled");

        if (chauffeEauEnabledValue != -1) {
